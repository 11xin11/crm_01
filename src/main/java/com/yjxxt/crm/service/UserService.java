...
public class UserService extends BaseService<User,Integer>{
    ...
    * */
    ...
    private void relaionUserRole(Integer userId, String roleIds) {
        ...
        //删除原来的角色
        if (count>0){
            AssertUtil.isTrue(userRoleMapper.deleteUserRoleByUserId(userId)!=count,"用户行角色删除失败！");
        }
    ...
    private void checkUser(User user) {
        ...
        if(user.getId()==null){
            //添加操作
            //判断是否存在
            AssertUtil.isTrue(temp!=null,"用户名已经被使用，请重新输入！");
        }else {
            //修改操作(数据存在，id不是当前数据本身)
            AssertUtil.isTrue(temp!=null&&!temp.getId().equals(user.getId()),"用户名已经被使用，请重新输入！");
        }
        AssertUtil.isTrue(StringUtils.isBlank(user.getEmail()),"邮箱不能为空");
        AssertUtil.isTrue(!PhoneUtil.isMobile(user.getPhone()),"输入合法的手机号");
    }
